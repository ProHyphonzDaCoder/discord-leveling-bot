...
let deleteLevel = sql.prepare(
    "DELETE FROM levels WHERE user = ? AND guild = ?"
);

let getBg = sql.prepare(
    "SELECT bg FROM background WHERE user = ? AND guild = ?;"
);
let setBg = sql.prepare(
    "INSERT OR REPLACE INTO background (user, guild, bg) VALUES (@user, @guild, @bg);"
);
...
module.exports = {
    ...
    getLevel,
    setLevel,
    deleteLevel, // queries for levels
    getBg,
    setBg, // queries for backgrounds
    blacklist,
    insertLevel,
    serverSettings,
    channelLevel,
