	...
	async execute(interaction) {
		...
		let buildTable = async () => {
			...
			for (var i = 1 in myList) {
				...
				if (!specifiedUser) {
					interaction.client.users.fetch(myList[i].user).then(cachedUser => specifiedUser = cachedUser);
				}
				...
				context.fillText(`LVL ${myList[i].level}`, 60 + rankWidth + 20 + longestEntry + 20, (i * 55) + (50 / 2));
