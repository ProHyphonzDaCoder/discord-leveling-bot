...
module.exports = {
	...
	async execute(interaction) {
		...
		var valid =
			/^(http|https):\/\/[^ "]+$/.test(background) &&
			(background.endsWith(".png") || background.endsWith(".jpg") || background.endsWith("jpeg"));
		if (!valid)
			return interaction.followUp(
				"Please enter a valid image URL. It must use HTTP or HTTPS and be a PNG, JPG, or JPEG image. It must not have a `?` or `&` at the end."
			);

		try {
			sql
				.prepare("INSERT OR REPLACE INTO background (user, guild, bg) VALUES (?, ?, ?);")
